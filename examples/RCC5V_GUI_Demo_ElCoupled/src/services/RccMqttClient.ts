// ______RccMqttClient.ts_________________________khartinger_____
// 2021-12-26: new
// 2023-01-22: add classes, update RccBase.vue, RccBaseController.ts
// 2023-09-02: update to mqtt 5
import { connect, MqttClient } from 'mqtt/dist/mqtt.min'
import { RccBaseController } from '@/controller/RccBaseController'
...
export class RccMqttClient {
  // ---------basic properties----------------------------------
  public client: MqttClient | null = null
  public controller: Array<RccBaseController> = []
  ...
  public disconnect (): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.client.end(true, {}, (err: any) => {
        if (err) {
          // console.log('RccMqttClient:disconnect: ', 'failed!')
          return reject(new Error('Could not disconnect'))
        }
        // console.log('RccMqttClient:disconnect: ', 'success!')
        resolve()
      })
    })
  }
  ...
  private subscribe_ (): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.client) {
        // console.log('RccMqttClient:subscribe: failed - not connected! ' + this.privateMqttSubscription.topic)
      ...
      this.client.subscribe(this.subTopic, { qos: this.subQos }, (err: any) => {
        if (err) {
          // console.log('RccMqttClient:subscribe: failed! ' + this.subTopic)
        ...
        // console.log('RccMqttClient:subscribe: success! ' + this.privateMqttSubscription.topic)
        resolve()
      })
    })
  }
  ...
  public unsubscribe (): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.privateMqttSubscription.subscribed = false
      if (!this.client) {
        // console.log('RccMqttClient:unsubscribe: failed - not connected! ', this.privateMqttSubscription.topic)
      ...
      if (subscribedOld) {
        this.client.unsubscribe(this.privateMqttSubscription.topic, {
          ...
          if (err) {
            // console.log('RccMqttClient:unsubscribe: failed! ', this.privateMqttSubscription.topic)
            return reject(new Error('Could not unsubscribe topic ' + this.privateMqttSubscription.topic))
          }
          // console.log('RccMqttClient:unsubscribe: success! ', this.privateMqttSubscription.topic)
          resolve()
        })
      }
    })
  ...
  public async hostSubscribe (host: string, topicSubscribe: string): Promise<boolean> {
    ...
    try {
      ...
      console.error('RccMqttClient.hostSubscribe: Error ' + err)
      return false
    }
    return true
  }
  ...
  public registerController (controller: RccBaseController): void {
  ...
  public async reconnectBroker (): Promise<boolean> {
    try {
      ...
      console.error('RccMqttClient.reconnectBroker: Error ' + err)
      return false
    }
    return true
  }
  ...
  public init (): void {
    console.log('RccMqttClient.init')
  }
}
