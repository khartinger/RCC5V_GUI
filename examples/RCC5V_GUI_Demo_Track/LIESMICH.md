...
Letzte &Auml;nderung: 19.1.2026 <a name="up"></a><br>   
<h1>RCC5V_GUI_Demo_Track: Gleissymbole f&uuml;r RCC5V-GUIs</h1>
...
# 1. Einleitung
Dieses Dokument beschreibt die Verwendung von Gleissymbolen in RCC5V-GUI-Anwendungen.   
Das Bild zeigt m&ouml;gliche Symbole:
![Gleissymbole](./images/300_symbol_track1.png 
"Gleissymbole")   
_Bild 1: Gleissymbole_   

Die Nutzung der Symbole wird in folgenden Kapiteln beschrieben.   
...
Jedes Gleissymbol besitzt die F&auml;higkeit, MQTT-Nachrichten zu senden und zu empfangen, um so den Fahrstrom ein- und auszuschalten. Um diese M&ouml;glichkeit zu testen, ist folgende Hardware erforderlich:   
* Ein Gleis, dessen Stromzuf&uuml;hrung mit einem RCC-Block abgeschaltet werden kann.   
  _Empfehlung_: Verwendung des abschaltbaren Gleises des RCC-Blocktesters mit der DCC-Adresse 41. Der Blocktester wird auf [https://github.com/khartinger/RCC5V/tree/main/examples/blocktester](https://github.com/khartinger/RCC5V/tree/main/examples/blocktester)   
  beschrieben.   
...
Um alle M&ouml;glichkeiten dieses Projektes auszuprobieren, wird folgende Software ben&ouml;tigt:   
...
Um die Funktionalit&auml;t der Gleissymbole herzustellen, sind weitere Schritte notwendig:   
...
Klickt man im Browser bei einem rot gef&auml;rbten Gleis in die obere H&auml;lfte des Feldes, so f&auml;rbt sich das Gleis gr&uuml;n und im RCC-Blocktester wird der Strom im Gleis eingeschaltet.   
...
* Das Verzeichnis `router` enth&auml;lt die Datei `index.ts`, die f&uuml;r das Weiterleiten der Links in der ersten Zeile (Kopfzeile mit `MainView` und `About`) verantwortlich ist.   
...
## 5.1 Syntax   
Gleissymbole werden in der Datei `MainView.vue` mit dem Tag `<RccTrack ...>` gezeichnet, wobei folgende Parameter erforderlich bzw. m&ouml;glich sind:   
...
8 ... rechts unten   
0 ... Keine Richtung   
Einstellige Werte oder ganze Zahnerwerte ergeben einen Prellbock.   
#### _Beispiele_
15 ... gerades, waagrechtes Gleis   
25 ... Gleis, das gerade (waagrecht) von links kommt (5) und nach rechts oben (2) abbiegt.   

Im _Bild 1_ sind alle M&ouml;glichkeiten mit ihren Zahlenwerten dargestellt.
...
* `iTrack1State`: Status des Gleises. Ein Wert -99 bedeutet "unbestimmt", 0 bedeutet "Fahrstrom abgeschaltet" und 1 bedeutet "Fahrstrom eingeschaltet". Der Wert wird zB bei der Anzeigefarbe f&uuml;r das Symbol verwendet.   
...
* `payloadInvert`: Damit kann die Bedeutung des Inhalts einer empfangenen Nachricht umgekehrt werden. Im Normalfall passiert das nicht und der Wert ist &uuml;blicherweise `false`.
...
## 5.5 Beispiele
1. An der Rasterposition 0/5: Gerades Gleis ohne Rahmenangabe, Beschriftung und Funktion   
`<RccTrack1 :x="0*dx" :y="5*dy" sid="tk0" dir="15"></RccTrack1>`   

2. An der Rasterposition 1/5: Gleis waagrecht von links nach rechts unten ohne Rahmen, ohne Beschriftung und Funktion   
...
6. An der Rasterposition 5/4: Gleis von 
links unten nach rechts unten, ohne 
Funktion und Beschriftung, mit Rahmen Nummer 3.   
`<RccTrack1 :x="5*dx" :y="4*dy" sid="tk0" dir="68" :border="3"></RccTrack1>`   

7. An der Rasterposition 6/5: Gleis von 
links oben nach rechts unten ohne 
Funktion und Beschriftung, mit Standardrahmen.   
``   
...
Davon gibt zwei Arten f&uuml;r die beiden schr&auml;gen 
Richtungen "2" und "4".   
