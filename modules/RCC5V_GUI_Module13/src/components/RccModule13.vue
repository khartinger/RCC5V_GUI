...
<template>
  ...
  <!-- row 1 ----------------------------------------------- -->
  <RccTrack1    :x="x0+1*dx" :y="y0+0*dy" sid="tk0" dir="48" :border="border"></RccTrack1>
  <!-- row 2 ----------------------------------------------- -->
  <RccTrack1     :x="x0+0*dx" :y="y0+1*dy" sid="tk2" dir="15" header="[ON]" headeralign="C" footer="133" footeralign="C" :border="border"></RccTrack1>
  <RccUncoupler1 :x="x0+1*dx" :y="y0+1*dy" sid="uc1" dir="15" header="[ON]" headeralign="C" footer="134" footeralign="C" :border="border"></RccUncoupler1>
  <RccTurnout3Way1 :x="x0+2*dx" :y="y0+1*dy" sid="t3w1" dir="4" header="132" headeralign="R" footer="131" footeralign="R" :border="border"></RccTurnout3Way1>
  <!-- row 3 ----------------------------------------------- -->
  <RccTrack1    :x="x0+1*dx" :y="y0+2*dy" sid="tk0" dir="26" :border="border"></RccTrack1>
  ...
  <text class="ciFont1" :x="x0-0.48*dx" :y="y0-0.3*dy">Module 13</text>
</g>
</template>
...
watchEffect(() => {
  if(ciMqttClientInstance.mqttState.connected) {
    ciMqttClientInstance.publish('rcc/demo1/get', 'status', false, 0).catch((e) => { console.error('RccModule13: ERROR:', e) })
  }
})
