...
<template>
  ...
  <!-- row 1 ----------------------------------------------- -->
  <RccUncoupler1 :x="x0+0*dx" :y="y0+0*dy" sid="uc1" dir="15" footer="113 [OFF]" footeralign="C" :border="border"></RccUncoupler1>
  <RccTrack1     :x="x0+1*dx" :y="y0+0*dy" sid="tk1" dir="58" header="[ON]" headeralign="R" footer=" 112 [OFF]" footeralign="L" :border="border"></RccTrack1>
  <!-- row 2 ----------------------------------------------- -->
  ...
  <RccTurnout1   :x="x0+2*dx" :y="y0+1*dy" sid="to1" type="5R" header="111 " headeralign="R" :border="border"></RccTurnout1>
  <!-- row 3 ----------------------------------------------- -->
  <!-- rail connectors, isolators and module name----------- -->
  <RccTrackIso1 :x="x0+1.5*dx" :y="y0+0.5*dy" sid="con0" dir="8"></RccTrackIso1>
  <text class="ciFont1" :x="x0-0.48*dx" :y="y0-0.3*dy">Module 11</text>
</g>
</template>
...
watchEffect(() => {
  if(ciMqttClientInstance.mqttState.connected) {
    ciMqttClientInstance.publish('rcc/demo1/get', 'status', false, 0).catch((e) => { console.error('RccModule11: ERROR:', e) })
  }
})
