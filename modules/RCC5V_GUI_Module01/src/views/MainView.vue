...
<template>
  <svg width="100%" :viewBox="viewbox">
    ...
    <RccModule01 :x0="xB" :y0="yB" sid="m01" :border="border" :color="color" :route="routeXm01" />
  </svg>
<br>
<table><tr><td>
<button
...
<script setup lang="ts">
  ...
  const topicGet = 'rcc/module01/get'
  const topicSet = 'rcc/module01/set/'

  const route1dcc = '12/1'
  const route1m01 = ' 0/1,1/1'
  const route2dcc = '12/0'
  const route2m01 = '0/1,1/0'
  const route3dcc = '12/1,11/1'
  const route3m01 = ' 0/0,1/0'

  const routesOn = [
    { id: 1, label: '1', sRouteMxx: route1m01, sRouteDcc: route1dcc },
    { id: 2, label: '2', sRouteMxx: route2m01, sRouteDcc: route2dcc },
    { id: 3, label: '3', sRouteMxx: route3m01, sRouteDcc: route3dcc },
  ]
  const routesOff = [
    { id: 1, label: '1', sRouteMxx: route1m01, sRouteDcc: route1dcc },
    { id: 2, label: '2', sRouteMxx: route2m01, sRouteDcc: route2dcc },
  ...
  const routeXm01 = ref('') //         Route for <RCCModule01 ..>
  const color = ref(geof.colorNone) // Color for <RCCModule01 ..>
  ...
  const nx = m01.nx // symbols in x direction
  const ny = m01.ny // symbols in x direction
  ...
  async function sendRouteWithDelay(aRoute1: string[]): Promise<void> {
    for (const msg of aRoute1) {
      ...
      try {
        ...
        console.error('Module01: ERROR:', e)
      }
      // wait 0,2 secs------------------------------------------
      await sleep(200)
    }
  ...
  function onClkRouteOn(n: number): void {
    ...
    routeXm01.value = sRouteXY // for <RCCModule 01 ..>
    color.value = geof.colorRoute // for <RCCModule 01 ..>
  ...
  function onClkRouteOff(n: number): void {
    ...
    if(!sRouteXY) return
    // console.log('Button clicked:', n + ', sRouteXY=' + sRouteXY)
    routeXm01.value = sRouteXY // Route for <RCCModule01 ..>
    color.value = '-' //          Color for <RCCModule01 ..>
  }
/*
  // ____waiting for MQTT connection, then get status___________
  watchEffect(() => {
    if (rccMqttClientInstance.mqttState.connected) {
      rccMqttClientInstance.publish(topicGet, 'status', false, 0).catch((e) => { console.error('Module01: ERROR:', e) })
    }
  })
*/
</script>
